...
function createReporterConfigFile(path) {
  const defaultReporters = [
    'cypress-parallel-extended/json-stream.reporter.js',
    'cypress-parallel-extended/simple-spec.reporter.js'
  ];

  const reporterEnabled = settings.reporter
    ? ['cypress-parallel-extended/json-stream.reporter.js', settings.reporter]
    : defaultReporters;
  ...
  reporterEnabled.forEach((reporter) => {
    let optionName;
    if (reporter === 'cypress-parallel-extended/json-stream.reporter.js') {
      optionName = 'cypressParallelExtendedJsonStreamReporterJsReporterOptions';
    } else if (reporter === 'cypress-parallel-extended/simple-spec.reporter.js') {
