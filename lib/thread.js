...
async function createReporterConfigFile(index) {
  ...
  const defaultReporters = [
    'cypress-parallel/json-stream.reporter.cjs',
    'cypress-parallel/simple-spec.reporter.cjs',
  ];
  const reporterEnabled = settings.reporter
    ? ['cypress-parallel/json-stream.reporter.cjs', settings.reporter]
    : defaultReporters;
  ...
  reporterEnabled.forEach((reporter) => {
    let optionName;
    if (reporter === 'cypress-parallel/json-stream.reporter.cjs') {
      optionName = 'cypressParallelExtendedJsonStreamReporterJsReporterOptions';
    } else if (reporter === 'cypress-parallel/simple-spec.reporter.cjs') {
