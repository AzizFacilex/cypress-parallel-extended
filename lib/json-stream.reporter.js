...
function JSONStreamCustom(runner, options) {
  ...
  var self = this;
  const resultsPath = options.reporterOptions?.reportDir || 'runner-results';  // Use runnerResults here
  ...
  runner.on(EVENT_SUITE_END, function () {
    writeFile(cleanStatistics(), resultsPath);
  });
}
...
function writeFile(statistics, resultsPath) {
